...
public partial class MyInventory : Container
{
	...
	private int paddingLR; //The (Container X % (boxDiameter + padding)) / 2 : extra space that cannot fit a box; split between the left and right sides
	...
	public MyInventory(Container parentContainer, Sprite2D boxSprite)
	{
		...
		_generateBoxesVisual(boxRequestCount);//TODO make a nice version using a more accurate count instead of a random int.
	}
	/**
	...
	public Sprite2D MouseCoordToBox(Vector2 coord)
	{
		...
		Vector2 boxDiscreteCoord = new Vector2((int)(coord.X / (_boxDiameter + someValue)), (int)(coord.Y / (_boxDiameter + someValue))); //TODO needs a limit on the click location
	...
	{
		var result = count;
		var nextBoxCoordinate = new Vector2((float)(_boxDiameter / 2 + paddingLR), (float)(_boxDiameter / 2  + _boxPadding)); //Shift 
		
		for (var i = 0; i < count; i++)
		{
			//TODO-Bug one extra horizontal line of boxes is being created
			
			if (nextBoxCoordinate.X + _boxDiameter + (_boxDiameter / 2) > (_me.Size.X - paddingLR)) //if the next box's X coordinate is out of bounds.
			{
				nextBoxCoordinate.X = (float)(_boxDiameter / 2 + paddingLR);
				nextBoxCoordinate.Y += _boxDiameter + _boxPadding;
			}
			
			if (nextBoxCoordinate.Y + (_boxDiameter / 2) > _me.Size.Y) //if the next box's Y coordinate is out of bounds.
